...
import { validateWebreelConfig, loadWebreelConfig, parseSchemaVersion, resolveConfigPath, formatValidationErrors, buildLineMap, getConfigDir, filterVideosByName, } from "../config.js";
...
describe("validateWebreelConfig videos", () => {
    ...
    it("validates video viewport dimensions", () => {
        const errors = validateWebreelConfig(wrapVideo({
            url: "u",
            viewport: { width: -1, height: 0 },
            steps: [],
        }));
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.viewport.width" }));
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.viewport.height" }));
    });
});
describe("validateStep", () => {
    ...
    it("rejects unknown action", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].action" }));
    });
    ...
    it("validates pause requires ms", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].ms" }));
    });
    ...
    it("validates key requires non-empty key", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].key" }));
    });
    it("validates drag from requires text or selector", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].from" }));
    });
    it("validates drag to requires text or selector", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].to" }));
    });
    ...
    it("validates type requires non-empty text", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].text" }));
    });
    ...
    it("validates scroll x must be a number", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].x" }));
    });
    ...
    it("validates screenshot requires non-empty output", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].output" }));
    });
    ...
    it("rejects non-string description", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.steps[0].description" }));
    });
});
...
describe("include validation", () => {
    ...
    it("rejects empty string in include array", () => {
        ...
        expect(errors).toContainEqual(expect.objectContaining({ path: "videos.x.include[0]" }));
    });
});
...
describe("loadWebreelConfig", () => {
    ...
    it("loads a config with videos object", async () => {
        ...
        try {
            writeFileSync(resolve(dir, "webreel.config.json"), JSON.stringify({
                $schema: "https://webreel.dev/schema/v1.json",
                videos: {
                    hero: {
                        url: "https://example.com",
    ...
    it("rejects config without videos object", async () => {
        mkdirSync(dir, { recursive: true });
        try {
            writeFileSync(resolve(dir, "invalid.json"), JSON.stringify({
                url: "https://example.com",
                steps: [{ action: "pause", ms: 100 }],
            }));
            await expect(loadWebreelConfig(resolve(dir, "invalid.json"))).rejects.toThrow("Invalid config");
        }
        finally {
            cleanup();
        }
...
describe("filterVideosByName", () => {
    ...
    it("throws when a name is not found", () => {
        expect(() => filterVideosByName(videos, ["hero", "missing"])).toThrow("Video(s) not found: missing");
    });
});
